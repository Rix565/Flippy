...
class CreatorIDFileResource(resource.Resource):
    ...
    def render(self, request):
        ...
        if filetype in "ppm":
            ...
            if "x-hatena-star-count" not in headers:
                ServerLog.write("%s got 403 when requesting %s without a X-Hatena-Star-Count header" % (
                    request.getClientIP(), path), Silent)
                request.setResponseCode(403)
            ...
            if (amount < 1) or (amount > 65535):
                ServerLog.write("%s got 403 when requesting %s with an invalid X-Hatena-Star-Count header" % (
                    request.getClientIP(), path), Silent)
                request.setResponseCode(403)
            ...
            # report success
            ServerLog.write("%s added %i stars to %s/%s.ppm" % (request.getClientIP(), amount, creator, file[:-5]),
                            Silent)
            return "Success"
        elif filetype == "dl":
            path = "/".join(request.path.split("/")[3:])
    ...
    def GenerateDetailsPage(self, CreatorID, filename):  # filename without ext
        flipnote = Database.GetFlipnote(CreatorID,
                                        filename)  # flipnote = [filename, views, stars, green stars, red stars, blue stars, purple stars, Channel], all probably strings
        if not flipnote:
        ...
        if tmb.OriginalAuthorID <> tmb.EditorAuthorID or tmb.OriginalFilename <> tmb.CurrentFilename:
            if Database.FlipnoteExists(tmb.OriginalAuthorID, tmb.OriginalFilename[:-4]):
                Spinnoff = SpinoffTemplate1.replace("%%CreatorID%%", tmb.OriginalAuthorID).replace("%%Filename%%",
                                                                                                   tmb.OriginalFilename[
                                                                                                   :-4])
        ...
        #content = '<a href="http://flipnote.hatena.com/ds/v2-xx/%s/profile.htm?t=260&pm=80\">%s</a>' % (
        #   CreatorID, tmb.Username)
        content = "%s" % tmb.Username
        ...
        return DetailsPageTemplate.replace("%%CreatorID%%", CreatorID).replace("%%Filename%%", filename).replace(
            "%%CommentCount%%", Comments).replace("%%Spinoff%%", Spinnoff).replace("%%AccountInfo%%",
                                                                                   accountinfo).replace(
            "%%PageEntries%%", PageEntrySeparator.join(Entries))
...
AccountInfo = """
        <div class="notice2" align="center">
            The accounts page is in developpement.
		</div>"""
PageEntryTemplate = """
			<tr>
				<th width="90">
